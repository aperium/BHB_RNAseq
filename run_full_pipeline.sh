...
# Usage: bash run_full_pipeline.sh [ACCOUNT]
#   ACCOUNT (optional): SLURM account to use (default: $USER)

# Set SLURM account (use provided argument or default to $USER)
SLURM_ACCOUNT="${1:-$USER}"
...
JOB2=$(sbatch --parsable --account=${SLURM_ACCOUNT} --dependency=afterok:$JOB1 02_multiqc_raw.slurm)
...
JOB3=$(sbatch --parsable --account=${SLURM_ACCOUNT} --dependency=afterok:$JOB2 03_trimmomatic.slurm)
echo "Step 3 (Trimmomatic): Job ID $JOB3 (waits for $JOB2) - 42 array tasks"

# Submit Step 4: FastQC trimmed (depends on Trimmomatic)
JOB4=$(sbatch --parsable --account=${SLURM_ACCOUNT} --dependency=afterok:$JOB3 04_fastqc_trimmed.slurm)
echo "Step 4 (FastQC trimmed): Job ID $JOB4 (waits for $JOB3)"

# Submit Step 5: MultiQC trimmed (depends on FastQC trimmed)
JOB5=$(sbatch --parsable --account=${SLURM_ACCOUNT} --dependency=afterok:$JOB4 05_multiqc_trimmed.slurm)
...
JOB6=$(sbatch --parsable --account=${SLURM_ACCOUNT} --dependency=afterok:$JOB0 06_build_star_index.slurm)
echo "Step 6 (STAR index): Job ID $JOB6 (waits for $JOB0)"

# Submit Step 7: STAR alignment (depends on STAR index AND trimmed reads)
JOB7=$(sbatch --parsable --account=${SLURM_ACCOUNT} --dependency=afterok:$JOB6,afterok:$JOB3 07_star_align.slurm)
...
JOB8=$(sbatch --parsable --account=${SLURM_ACCOUNT} --dependency=afterok:$JOB7 08_multiqc_alignment.slurm)
echo "Step 8 (MultiQC align): Job ID $JOB8 (waits for $JOB7)"

# Submit Step 9: featureCounts (depends on STAR alignment)
JOB9=$(sbatch --parsable --account=${SLURM_ACCOUNT} --dependency=afterok:$JOB7 09_featureCounts.slurm)
echo "Step 9 (featureCounts): Job ID $JOB9 (waits for $JOB7)"

# Submit Step 10: DESeq2 (depends on counts)
JOB10=$(sbatch --parsable --account=${SLURM_ACCOUNT} --dependency=afterok:$JOB9 10_run_deseq2.slurm)
